...
export class DateTimeModel implements NgbDateTimeStruct {
  ...
  public static now(): DateTimeModel {
  ...
  public static fromDate(date:any): DateTimeModel {
  ...
  public toString(): string | null {
    if (this.isInteger(this.year) && this.isInteger(this.month) && this.isInteger(this.day)) {
      ...
      const isoString = `${pad(parseInt(year))}-${pad(parseInt(month))}-${pad(parseInt(day))}T${pad(parseInt(hour))}:${pad(parseInt(minute))}:${pad(parseInt(second))}${dif}${pad(tzo / 60)}:${pad(tzo % 60)}`;
      return isoString;
    }
